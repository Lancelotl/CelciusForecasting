...
def fetch(location_object):
    ...
    if r.ok:
        ...
        raise HttpError({"service": "Yr.no", "response": r.status_code})
...
def retrieve(location_object, target_local_time):
    ...
    if not metadata:
        raise BadResponse({"service": "Yr.no", "key": "model name=met_public_forecast"})
    try:
        issue_time = metadata["runended"]
    except KeyError:
        BadResponse({"service": "Yr.no", "key": "model/runended"})
    ...
    if not pointdata:
        raise BadResponse({"service": "Yr.no", "key": "product class=pointData"})
    ...
    if not forecast:
        raise BadResponse({"service": "Yr.no", "key": "time datatype=forecast"})
    try:
        temperature = forecast.find("location").find(
            name="temperature", attrs={"id": "TTT", "unit": "celsius"}
        )["value"]
    except KeyError:
        BadResponse({"service": "Yr.no", "key": "temperature/value"})
    if not temperature:
        BadResponse({"service": "Yr.no", "key": "temperature id=TTT"})
